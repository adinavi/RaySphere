...
Vec3 ray_color(const Ray& ray, const Vec3& light_position, const std::vector<Sphere>& spheres) {
...
void render_frame(const std::string& filename, int image_width, int image_height, const Vec3& light_position, const std::vector<Sphere>& spheres) {
    ...
    for (int j = image_height - 1; j >= 0; --j) {
        for (int i = 0; i < image_width; ++i) {
            ...
            Vec3 color = ray_color(ray, light_position, spheres);
...
int main() {
    ...
    for (int frame = 0; frame < frame_count; ++frame) {
        ...
        render_frame(filename, image_width, image_height, light_position, spheres);
